...
namespace Operating_System_Task
{
    class Command_string
    {
        ...
        string command_name;
        string[] words;
        public static Directory MoveToDir(string full_Path)
        {
            Directory d = null;
            string[] path = full_Path.Split("\\");
          
            if (path[0] == Program.root.Directory_Name)
            {

                Directory d1 = new Directory(path[0], 0x10, Program.current);

                for (int i = 1; i < path.Length; i++)
                {
                    int index = d1.search_about_Directory(path[i]);
                    if (index != -1)
                    {

                        d = new Directory(path[i], 0x10, d1);
                        d1 = d;
                       // d.GetDirectory_Entery();
                    }
                    else
                    {
        ...
        public static File_Entery MoveToFile(string full_Path)
        {
            File_Entery f = null;
            int name_start_index = full_Path.LastIndexOf("\\");
            string file_name = full_Path.Substring(name_start_index + 1);
            string path = full_Path.Substring(0, name_start_index + 1);
            Directory d = MoveToDir(path);
            d.Read_Directory();
            int index = d.search_about_Directory(file_name);
            if (index != -1)
            {
                ...
                f = new File_Entery(file_name, 0x00, first_cluster, Program.current, file_size);
                f.read_File_Content();
            }
            else Console.WriteLine("error");
            return f;
        ...
        public static void export_File(string source, string destination)
        {
            ...
            if (index != -1)
            {
                File_Entery f = MoveToFile(source);
                if (System.IO.Directory.Exists(destination))
                {
                    ...
                    // in real disk
                    File_Entery f1 = new File_Entery(source, 0x00, first_cluster, Program.current, file_size);
                    f1.read_File_Content();
        ...
        public static void import_File(string file_path)
        {
            ...
            if (File.Exists(file_path))
            {
                ...
                if (index == -1)
                {
                    ...
                    File_Entery f = new File_Entery(file_name, 0x00, first_cluster, Program.current, content, size);
                    f.write_File_Content();

                    Program.current.directory_File.Add(f);
        ...
        public static void delet_file(string file_name)
        {
            ...
            if (index != -1&& Program.current.directory_File[index].dir_attr == 0x00)//file
            {
                ...
                //string content;
                File_Entery f = new File_Entery(file_name, 0x00, first_cluster, Program.current, file_size);
                f.read_File_Content();
                f.Delet_file();
        ...
        public static void rename_directory(string old_Name,string new_Name)
        {
            ...
            if (index_old_Name != -1)
            {
                if (index_new_name == -1)
                {
                    ...
                    int size = Program.current.directory_File[index_old_Name].dir_file_size[0];
                    Directory_Entery d = new Directory_Entery(old_Name, 0x10, first_cluster,size);
        ...
        public static void change_directory(string name)
        {
            ...
            if (index != -1)
            {
                ...
                Console.WriteLine(Program.current.Directory_Name);
        ...
        public static void Dir(string name)
        {
            ...
            if (Program.current.Directory_Name == name)
            {
                ...
                for (int i = 0; i < x; i++)
                {
                    ...
                    {
                        file_size += Program.current.directory_File[i].dir_file_size[0];
            ...
            Console.WriteLine($"{count_file} file(s)   {file_size}   size ");
